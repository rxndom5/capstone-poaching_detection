...
consumer = KafkaConsumer(
    ...
    group_id='vehicle-detector-group' # Unique group ID
)
...
print("Vehicle detector service is running...")
for message in consumer:
    image_info = message.value
    image_id = image_info['image_id']
    image_data = base64.b64decode(image_info['image_data'])
    
    print(f"[Vehicle] Processing image ID: {image_id}")
    ...
    producer.flush()
    print(f"[Vehicle] Sent {len(detections)} detections for image ID: {image_id}")
